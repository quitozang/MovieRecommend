...
object MovieMain {
  def main(args: Array[String]): Unit = {
    ...
    val numUsers = ratings.map(_._2.user).distinct().count()  //去重
    ...
    val numPartitions = 4
    val training = ratings.filter(x => x._1 < 6)    //训练
      .values
      .union(myRatingsRDD) //注意ratings是(Int,Rating)，取value即可
      .repartition(numPartitions)                   //重分区
      .cache()
    val validation = ratings.filter(x => x._1 >= 6 && x._1 < 8)   //校验
      .values
      .repartition(numPartitions)
      .cache()
    val test = ratings.filter(x => x._1 >= 8).values.cache()    //测试
